#!/usr/bin/env python3
"""Add service photo JS functions to chef-profile-page.ts"""
with open('src/routes/chef-profile-page.ts', 'rb') as f:
    content = f.read()

# Find 'async function loadChefExperiences'
search = b'async function loadChefExperiences'
idx = content.find(search)
if idx == -1:
    print('ERROR: Could not find loadChefExperiences')
    exit(1)

# Insert before this line (at the newline before it)
newline_idx = content.rfind(b'\n', 0, idx)
insert_point = newline_idx + 1  # Start of the line

print(f'Inserting JS functions at byte {insert_point}')

# The JS functions to add
js_functions = b"""  html += '    var currentServiceId = null;';
...
new_content = content[:insert_point] + js_functions + content[insert_point:]

with open('src/routes/chef-profile-page.ts', 'wb') as f:
    f.write(new_content)

print(f'Inserted {len(js_functions)} bytes of JS functions')